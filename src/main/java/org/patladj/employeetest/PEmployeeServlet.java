...
public class PEmployeeServlet extends HttpServlet {

    PTaskmanService employeeService = new PTaskmanService();
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (action!=null){
            ...
            List<PTaskman> result = employeeService.getAllEmployees();
    ...
    private void searchEmployeeById(HttpServletRequest req, HttpServletResponse resp)
        ...
        PTaskman employee = null;
        try {
            employee = employeeService.getEmployee(idEmployee);
        } catch (Exception ex) {
            Logger.getLogger(PEmployeeServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
        req.setAttribute("employee", employee);
        req.setAttribute("action", "edit");
        String nextJSP = "/jsp/new-ptaskman.jsp";
    ...
    private void searchEmployeeByName(HttpServletRequest req, HttpServletResponse resp)
        ...
        List<PTaskman> result = employeeService.searchEmployeesByName(employeeName);        
        forwardListEmployees(req, resp, result);
    }

    private void forwardListEmployees(HttpServletRequest req, HttpServletResponse resp, List employeeList)
            throws ServletException, IOException {
        String nextJSP = "/jsp/list-ptaskman.jsp";
        RequestDispatcher dispatcher = getServletContext().getRequestDispatcher(nextJSP);
    ...
    private void addEmployeeAction(HttpServletRequest req, HttpServletResponse resp)
        ...
        PTaskman employee = new PTaskman(name, lastName, birthday, role, department, email);
        long idEmployee = employeeService.addEmployee(employee);
        List<PTaskman> employeeList = employeeService.getAllEmployees();
    ...
    private void editEmployeeAction(HttpServletRequest req, HttpServletResponse resp)
        ...
        String email = req.getParameter("email");
        long idEmployee = Integer.valueOf(req.getParameter("idEmployee"));
        PTaskman employee = new PTaskman(name, lastName, birthday, role, department, email, idEmployee);
